"""REST API client for Sherlock audit contest platform."""
...
BASE_URL = "https://api.sherlock.xyz/v1"


class SherlockClient:
    """Async REST client for the Sherlock public API.
    ...
    def client(self) -> httpx.AsyncClient:
        if self._client is None:
            raise RuntimeError("SherlockClient must be used as async context manager")
        return self._client

    async def fetch_contests(self) -> list[UnifiedBounty]:
        """Fetch all Sherlock contests and normalize to UnifiedBounty."""
        try:
            resp = await self.client.get("/contests")
            resp.raise_for_status()
            data = resp.json()
        except Exception as e:
            log.error("sherlock.fetch_contests.error", error=str(e))
            return []
        ...
        for item in items:
            try:
                ...
                log.warning("sherlock.normalize_contest.error", id=item.get("id"), error=str(e))
        return normalized
    ...
    async def fetch_contest_detail(self, contest_id: str) -> UnifiedBounty | None:
        ...
        try:
            ...
            log.error("sherlock.fetch_contest_detail.error", contest_id=contest_id, error=str(e))
            return None

        try:
            return self._normalize_contest(data)
        except Exception as e:
            log.warning("sherlock.normalize_detail.error", contest_id=contest_id, error=str(e))
            return None

    def _normalize_contest(self, raw: dict) -> UnifiedBounty:
        contest_id = str(raw.get("id", ""))
        return UnifiedBounty(
            id=f"sherlock-{contest_id}",
            platform=BountyPlatform.SHERLOCK,
            ...
            url=f"https://www.sherlock.xyz/contests/{contest_id}",
