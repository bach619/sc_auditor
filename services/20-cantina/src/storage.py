"""Simple JSON file storage for Cantina contests data."""
...
def save_contests(contests: list[dict[str, Any]]) -> bool:
    """Write all contests to /data/cantina/contests.json."""
    _ensure_dir()
...
def load_contests() -> list[dict[str, Any]]:
    """Read all contests from /data/cantina/contests.json."""
    _ensure_dir()
    path = DATA_DIR / "contests.json"
    if not path.exists():
        return []
...
def save_contest_detail(contest_id: str, detail: dict[str, Any]) -> bool:
    """Write contest detail to /data/cantina/contest_{id}.json."""
    _ensure_dir()
...
def load_contest_detail(contest_id: str) -> dict[str, Any] | None:
    """Read contest detail from /data/cantina/contest_{id}.json."""
    _ensure_dir()
    path = DATA_DIR / f"contest_{contest_id}.json"
    if not path.exists():
        return None
