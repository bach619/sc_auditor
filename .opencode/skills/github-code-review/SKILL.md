...
description: "Review PRs: diffs, inline comments via gh or REST API."
license: MIT
...
- Authenticated with GitHub (via `gh auth login` or `GITHUB_TOKEN` env var)
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    echo "GITHUB_TOKEN not set. Set it in your environment or authenticate via 'gh auth login'."
    echo "Continuing with git-only operations (local review will work, PR interactions will not)."
  fi
fi
...
# Now you can use read, grep, run tests, etc.
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Opencode\",
    \"comments\": [
...
```bash
# Make sure GITHUB_TOKEN is set or gh is authenticated
# See Prerequisites section at the top of this skill
```
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Opencode. Code looks clean — good test coverage, no security concerns."
...
In addition to inline comments, leave a top-level summary so the PR author gets the full picture at a glance. Use the review output format from Section 1.
